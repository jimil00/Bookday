...
+ 결제 완료 시 rental 테이블에 해당 사용자 정보 입력 후 결제 완료 팝업창 구현
+ JSTL <c:choose> 태그를 활용하여 이미 결제한 사용자에겐 결제하기 버튼을 이용 중 버튼 구현
### [책가방(장바구니) 페이지] - 담당
...
+ JSTL <c:forEach> 태그를 활용하여 책가방(장바구니) 리스트 출력
  
  + 책가방에 담겨 있는 총 데이터 갯수 출력
  + 사용자가 책가방(장바구니)에 담은 항목 출력
  + 담은 책이 없을 때 "책가방에 담은 작품이 없습니다." 출력

+ ajax를 활용하여 위시리스트 담기, 이미 담은 위시리스트 재담기 방지 기능
  
  + 위시리스트 담기 버튼 클릭 시 wishlist 테이블에 해당 작품 존재 여부 조회 후, 존재하지 않으면 wishlist 테이블에 추가
